...
int fr_switch_input_processed(vfrs_ctx_t *ctx, vfr_port_t *port, const fr_addr_t *addr_ptr, u8 *frame, size_t frame_len)
{
    ...
    /* Capture destuffed frame bytes starting at Q.922 Address.
     * For serial and named pipe: FCS (2 bytes) is included in the capture. */
    size_t cap_len = frame_len + ((port->transport == PORT_TRANS_SERIAL ||
                                   port->transport == PORT_TRANS_PIPE) ? 2 : 0);
    if (port->capture) {
