...
static int svc_uni_process_msg_locked(vfr_port_t *port, const u8 *msg_data, size_t len) {
    ...
    case Q933_MSG_CONNECT: {
        ...
        /* Parse DLCI and Connected Number (if present) */
        u32 resp_dlci = 0;
        int dlci_found = 0;
        ...
        while (off + 2 <= ie_total_len) {
            ...
            if (ie_id == Q933_IE_DLCI) {
                ...
                q933_parse_subaddress(&ie_start[off + 2], ie_len, call->connected_subaddr, sizeof(call->connected_subaddr), &call->connected_subaddr_len);
            } else if (ie_id == Q933_IE_LOW_LAYER_COMPAT) {
                call->llc_len = ie_len > 32 ? 32 : ie_len;
                memcpy(call->llc_data, &ie_start[off + 2], call->llc_len);
            } else if (ie_id == Q933_IE_USER_USER) {
                call->uu_len = ie_len > 136 ? 136 : ie_len;
                memcpy(call->uu_data, &ie_start[off + 2], call->uu_len);
            } else {
                /* Unrecognized IE (X.36 §10.6.7.4 / §10.10.7.4) */
                if ((ie_id & 0x10) == 0) {
        ...
        if (call->peer_port) {
            ...
            if (peer_call) {
                ...
                /* Propagate optional IEs (LLC/UU) to peer CONNECT */
                peer_call->llc_len = call->llc_len;
                if (call->llc_len > 0) {
                    memcpy(peer_call->llc_data, call->llc_data, call->llc_len);
                }
    ...
    case Q933_MSG_STATUS_ENQUIRY: {
        ...
        if (tx_len > 0) {
            ...
            LOG_INFO("SVC [Port %s CRV 0x%04X]: Received STATUS ENQUIRY -> Responded with STATUS (State: %s)",
                     port->name, call ? call->call_ref : 0, call ? q933_get_state_name(call->state) : "Null (N0)");
        }
        break;
    }
