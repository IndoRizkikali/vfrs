...
static int port_udp_send(vfr_port_t *port, const u8 *frame, size_t len)
{
    ...
    if (!port || port->fd == INVALID_SOCKET || !port->priv) return -1;
    priv = (udp_priv_t *)port->priv;

    mutex_lock(&port->mutex);
    ret = sendto(port->fd, (const char *)frame, (int)len, 0,
...
static int port_udp_recv(vfr_port_t *port, u8 *buf, size_t max_len)
{
    ...
    ret = recvfrom(port->fd, (char *)buf, (int)max_len, 0,
