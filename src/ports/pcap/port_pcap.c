...
int pcap_writer_init(pcap_writer_t *pw, const char *filename, u32 snaplen)
{
    ...
    /* Enable full sharing (READ/WRITE/DELETE) so Wireshark or other viewers
     * can open and inspect the active capture file without sharing violation errors */
    pw->fd = CreateFileW(wpath, GENERIC_WRITE,
    ...
    hdr.network = PCAP_LINKTYPE_FRELAY;  /* Frame Relay (107) */
...
#ifdef _WIN32
    /* Immediately flush header so Wireshark recognizes valid PCAP format */
...
int pcap_writer_write(pcap_writer_t *pw, const u8 *data, size_t len)
{
    ...
    if (!pw || pw->fd == INVALID_HANDLE_VALUE || !data) return -1;

    /* Get current UTC time with microsecond resolution */
#ifdef _WIN32
    FILETIME ft;
    ...
    /* Convert 100ns intervals since Jan 1, 1601 to Unix epoch (Jan 1, 1970) */
    uint64_t t = (uli.QuadPart >= 116444736000000000ULL) ? (uli.QuadPart - 116444736000000000ULL) : 0;
    ts_sec = (u32)(t / 10000000ULL);
    ts_usec = (u32)((t % 10000000ULL) / 10ULL);
#else
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    ts_sec = (u32)ts.tv_sec;
    ts_usec = (u32)(ts.tv_nsec / 1000);
#endif

    /* Clamp captured length to configured snapshot length and buffer ceiling */
    size_t cap_len = len;
    if (cap_len > pw->snaplen) {
        cap_len = pw->snaplen;
    }
    if (cap_len > 8192) {
        cap_len = 8192;
    ...
    rec_hdr.ts_sec = ts_sec;
    rec_hdr.ts_usec = ts_usec;
    rec_hdr.incl_len = (u32)cap_len;
    rec_hdr.orig_len = (u32)len;

    size_t total_write = sizeof(rec_hdr) + cap_len;
    memcpy(write_buf, &rec_hdr, sizeof(rec_hdr));
    memcpy(write_buf + sizeof(rec_hdr), data, cap_len);
    ...
    /* Flush to disk so Wireshark live capture view updates immediately */
#ifdef _WIN32
    FlushFileBuffers(pw->fd);
#endif
