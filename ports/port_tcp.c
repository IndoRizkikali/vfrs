 ...
 * followed by the frame data in two separate send() calls (matching
 * dynamips net_io_tcp_send which uses two write() calls). */
static int port_tcp_send(vfr_port_t *port, const u8 *frame, size_t len)
{
    ...
    ret = send(port->fd, (const char *)send_buf, (int)total_len, 0);
