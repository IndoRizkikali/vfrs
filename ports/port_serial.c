...
static thread_ret_t THREAD_CALL serial_reader_thread(void *arg)
{
    ...
    while (priv->running) {
        /* Read 1 byte at a time. Blocking read with timeouts */
        if (!ReadFile(priv->hCom, &byte, 1, &bytes_read, NULL)) {
            if (priv->running) {
                msleep(10);
            }
            continue;
...
static int port_serial_send(vfr_port_t *port, const u8 *frame, size_t len)
{
    ...
    /* Log to PCAP if capture is enabled (DLT_FRELAY expects destuffed bytes) */
    if (port->capture) {
    ...
    if (!WriteFile(priv->hCom, out_stuffed, (DWORD)total, &written, NULL)) {
        mutex_unlock(&port->mutex);
        return -1;
    }
